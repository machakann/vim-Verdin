...
let s:METHODREGEX = printf('\m\C^\s*fu\%%[nction!]\s\+\zs%s\.\h\w*([^)]*)', s:VARNAME)
...
let s:ARGREGEX = printf('\m\C^\s*fu\%%[nction]!\?\s\+\%%(%s\|%s\)(\s*\zs\%%(\h\w*\%%(,\s*\h\w*\)*\%%(,\s*\.\{3}\)\?\|\s*\.\{3}\)\ze\s*)', s:FUNCNAME, s:VARNAME)
...
let s:constants.SEARCHTIMEOUT = 500
